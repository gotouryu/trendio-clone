...
function sanitizeBrief(raw: unknown): ScriptBrief {
  ...
  return {
    ...
    industryDetails: toStr(b.industryDetails),
...
<industry_details>${b.industryDetails || "未指定。業種固有の価格、対象範囲、許諾、注意事項、法規制に関わる情報は作らない"}</industry_details>
...
const INDUSTRY_RISK_TERMS: Array<{ match: string[]; terms: string[] }> = [
  ...
  {
    match: ["EC", "物販", "D2C", "小売"],
    terms: ["話題沸騰", "大人気", "残りわずか", "ランキング1位", "レビュー星"],
  },
  {
    match: ["採用", "人材"],
    terms: ["必ず採用", "内定保証", "誰でも転職", "年収アップ保証"],
  },
  {
    match: ["宿泊", "観光"],
    terms: ["地域No.1", "最安値保証", "絶景確約", "必ず満足"],
  },
  {
    match: ["フィットネス", "ジム"],
    terms: ["必ず痩せる", "リバウンドしない", "脂肪燃焼保証", "短期間で激変"],
  },
  {
    match: ["クリニック", "歯科"],
    terms: ["必ず治る", "完治", "痛みゼロ", "絶対安全", "症例数"],
  },
  {
    match: ["介護", "福祉"],
    terms: ["認知症が進行しません", "歩けるようになります", "全員が笑顔", "終の棲家"],
  },
  {
    match: ["リフォーム", "工務店"],
    terms: ["追加費用なし", "どんな家でも", "すぐ終わる", "欠陥住宅"],
  },
  {
    match: ["自動車", "整備", "中古車"],
    terms: ["事故歴なし", "最安値", "即納", "故障しない", "燃費保証"],
  },
  {
    match: ["クリーニング", "家事代行"],
    terms: ["新品同様", "汚れが必ず落ちる", "完全除菌", "最短即日"],
  },
  {
    match: ["ペット", "動物"],
    terms: ["必ず治る", "絶対安全", "噛み癖が治る", "ストレスゼロ"],
  },
  {
    match: ["冠婚葬祭", "式場", "葬儀"],
    terms: ["必ず感動", "後悔しない", "最安プラン", "豪華すぎる"],
  },
  {
    match: ["金融", "保険"],
    terms: ["必ず得する", "元本保証", "損しない", "税金が必ず安くなる"],
  },
];
...
- 採用・人材: 業務内容、働く環境、社員の本音、応募前の不安解消を重視。架空の社員コメント、性別・年齢の不当な限定、内定保証、年収アップ保証は禁止。
- 宿泊・観光: 客室、食事、アクセス、季節性、予約導線を重視。未入力の景色、料理、周辺スポット、割引、最安値、地域No.1を作らない。
- フィットネス・ジム: 体験の流れ、安全性、設備、続けやすさ、会員制の安心感を重視。短期激変、体重減少保証、危険なフォーム指示、体型攻撃は禁止。
- クリニック・歯科: 診療内容、リスク説明、院内設備、予約前の不安解消を重視。医療広告ガイドラインに抵触する効果保証、体験談捏造、他院誹謗、品位を損なう価格訴求は禁止。
- 介護・福祉: 施設種別、利用条件、見学時の不安解消、尊厳、家族の判断材料を重視。医療・介護効果の保証、入居者のプライバシー侵害、不安の過剰煽りは禁止。
- リフォーム・工務店: 施工範囲、現地調査、素材、工程、安全性、相談導線を重視。未入力の価格、工期、補助金、耐震・断熱性能、劇的なBefore/Afterを作らない。
- 自動車販売・整備: 車両状態、点検内容、見積、来店予約、安心材料を重視。未入力の事故歴、燃費、納期、保証、値引き、最安表現を作らない。
- クリーニング・家事代行: 作業範囲、予約手順、素材や汚れへの注意、日常の困りごと解消を重視。汚れ除去や除菌の保証、過度な衛生断定は禁止。
- ペット関連: 施術・預かり・しつけの流れ、動物への配慮、飼い主の不安解消を重視。治療効果、安全保証、動物のストレスゼロ断定は禁止。
- 冠婚葬祭: 相談前の不安、進行、費用確認、家族の納得を重視。感動保証、最安訴求、悲しみや不安の過剰煽り、宗教・慣習の決めつけは禁止。
- 金融・保険: 相談前の不安解消、比較観点、リスク説明、問い合わせ導線を重視。元本保証、利益保証、節税断定、将来結果の断定は禁止。
...
- 例示、NG例、比喩、仮のユーザー像としても、入力されていない地名、駅名、価格、家賃、期間、人数、実績、口コミ、ランキング、受賞歴を出さない。
...
- 業種別の重要条件が入力されている場合は、全案の表現リスク回避、見せ方、CTAに必ず反映する
...
- NG例や説明例としても、未入力の固有地名、駅名、価格、期間、人数、実績、口コミ、ランキング、受賞歴を出さない
...
- 「絶対」「必ず」「誰でも」「確実」「保証」「No.1」「日本一」「業界初」「最安」「最高」など、根拠が必要な断定・最上級は入力に根拠がない限り使わない。
- 投稿テーマの範囲を勝手に広げない。例: 夜限定を深夜、相談を診断、効率化を完全自動化へ言い換えない。
- 美容、健康、整体、整骨、鍼灸、教育、士業、金融、保険、不動産、介護、福祉、ペットでは、効果保証、合格保証、収益保証、法的結果保証、医療・介護・しつけ効果の断定を避ける。
- 採用、人材、宿泊、観光、フィットネス、ジム、クリニック、歯科では、架空の社員コメント、景色や料理の捏造、体重減少保証、医療効果保証、患者体験談の捏造を避ける。
- リフォーム、工務店、自動車、整備、クリーニング、家事代行、冠婚葬祭では、未入力の価格、工期、納期、車両状態、事故歴、除菌効果、感動保証、最安表現を避ける。
</hard_ng_rules>
...
- 業種別の重要条件に撮影許諾、価格条件、対象範囲、注意事項、法規制に関わる情報がある場合は、映像指示、テロップ、CTAに必ず反映する
