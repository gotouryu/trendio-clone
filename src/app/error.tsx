...
export default function ErrorPage({
  ...
  const { t } = useI18n();
  useEffect(() => {
    // 本番でも開発でも console には出す(=Vercel ログで追跡可能)
    // 画面表示は dev のみ
    console.error("App error boundary:", error);
  }, [error]);

  // Phase 4 修正(C7):本番では error.message を画面表示しない
  // (=Anthropic SDK のエラー文字列に API key prefix が含まれるケース等を防止)
  ...
  return (
    <div className="min-h-screen flex items-center justify-center auth-bg px-4">
      <div className="bg-white border border-gray-100 rounded-2xl p-10 max-w-lg w-full">
        ...
        <p className="text-sm text-gray-500 mb-4">{t("error.desc")}</p>
        {isDev && error.message && (
